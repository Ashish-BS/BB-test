...
export const fetchBlogs = async (filter?: {[key:string]:any}):Promise<BlogReturnType> => {
	let url = `${process.env.NEXT_PUBLIC_CONTENT_API_URL}/posts?populate=*&sort=publishDate:desc&`;
	if(filter){
...
export const fetchBlog= async (slug: string):Promise<BlogReturnType> => {
	return await fetchGet(`${process.env.NEXT_PUBLIC_CONTENT_API_URL}/posts?populate=*&filters[slug][$eq]=${slug}`);
};
